<!--
  EDITING GUIDE
  =============
  This README is a single-file content artifact. Owner-editable regions
  are bracketed by `EDIT:*` / `END:*` comment markers. Edit between the
  markers only; leave surrounding layout HTML untouched. Every listed
  region maps to a contract in specs/001-profile-readme-revamp/.

  - EDIT:HERO-TAGLINE     — Typing SVG lines. Line 1 is the hero tagline
                            (≤60 chars, kept literally as "Senior Software
                            Engineer, clean code and AI enthusiast"); lines
                            2–3 are adjunct (tech focus, writing pointer).
                            Must agree semantically with the hero capsule
                            text and the hero alt text.
  ...
  - EDIT:TECH-BADGES      — 3–9 shields.io badges, `style=for-the-badge`
                            uniformly. Each badge carries a valid
                            simple-icons `logo=` slug and alt text equal
                            to the tech name.
  - EDIT:FEATURED-PROJECTS — 2–6 curated projects, most-worth-seeing
                             first. Each entry renders as
                             `[name](url) — one-line description`,
                             ≤120 chars per line. No auto-generated repo
                             dump.
  - EDIT:WRITING-LIST     — Hand-curated writing only (no feed, no
                            auto-fetch). Series-clustered items render on
                            a single bullet with arrow-chain links
                            `[A](…) → [B](…) → [C](…)`; standalone items
                            render as `[title](url)` plus optional
                            description.
  - EDIT:CONTACT-LINKS    — Contact/social badges (shields.io) in a
                            single `<p>` block. Order: LinkedIn, dev.to,
                            Stack Overflow, Email.
-->

<p align="center">
  <img
    src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=12&height=220&section=header&text=Tiziano%20Basile&fontSize=64&fontColor=fff&animation=fadeIn&fontAlignY=38"
    alt="Tiziano Basile — Senior Software Engineer, clean code and AI enthusiast"
  />
</p>

<p align="center">
  <em>Motion-sensitive? GitHub doesn't apply <code>prefers-reduced-motion</code> to images, so enable your OS reduced-motion setting and <a href="#selected-work">jump to the static Selected Work section</a> to skip the animations below.</em>
</p>
...
<p align="center">
  <img
    src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=12&height=120&section=footer&reversal=true"
    alt="End of README"
  />
</p>
